...
async def randomchannel(
    tochat, channel, range1, range2, caption=None, client=mmo_bot
):
...
async def _insta_login():
    if "insta_creds" in mmo_bot._cache:
        return mmo_bot._cache["insta_creds"]
    ...
    if username and password:
        ...
        try:
            ...
            mmo_bot._cache.update({"insta_creds": cl})
...
async def get_insta_code(username, choice):
    ...
    async with asst.conversation(mmo_bot.uid, timeout=60 * 2) as conv:
