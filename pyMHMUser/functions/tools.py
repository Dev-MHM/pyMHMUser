...
async def metadata(file):
    ...
    if info.get("AudioCount"):
        ...
        data["performer"] = (
            info.get("Performer") or udB.get_key("artist") or mmo_bot.me.first_name
        )
...
async def get_chatbot_reply(message):
    ...
    req_link = chatbot_base.format(
        mmo_bot.me.first_name or "MHMuser user",
        message,
    )
    try:
...
def telegraph_client():
    ...
    gd_name = mmo_bot.full_name
    short_name = gd_name[:30]
    profile_url = (
        f"https://t.me/{mmo_bot.me.username}" if mmo_bot.me.username else None
    )
    try:
