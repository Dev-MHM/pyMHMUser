...
if run_as_module:
    ...
    async def heroku_logs(event):
        ...
        ok = app.get_log()
        with open("MHMuser-heroku.log", "w") as log:
            log.write(ok)
        await event.client.send_file(
            event.chat_id,
            file="MHMuser-heroku.log",
            thumb="resources/extras/MHMuser.jpg",
            caption="**MHMuser Heroku Logs.**",
        )

        os.remove("MHMuser-heroku.log")
        await xx.delete()
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>MHMuser {mmo_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"MHMuser {mmo_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
