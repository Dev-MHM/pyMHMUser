...
from ..version import mmo_version as ult_ver
...
def mmo_cmd(
    pattern=None, manager=False, mmo_bot=mmo_bot, asst=asst, **kwargs
):
    ...
    def decor(dec):
        async def wrapp(ult):
            ...
            try:
                ...
                await mmo_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await mmo_bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                ...
                ftext = (
                    "**MHMuser Client Error:** `Forward this to` @MHMUser\n\n"
                )
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**MHMuser Client Error:** `Forward this to` @MHMUser\n\n",
                        )
                else:
