...
async def autobot():
    ...
    if who.username:
        ...
        username = "mmo_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    await mmo_bot(UnblockRequest(bf))
    await mmo_bot.send_message(bf, "/cancel")
    await asyncio.sleep(1)
    await mmo_bot.send_message(bf, "/newbot")
    await asyncio.sleep(1)
    isdone = (await mmo_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("That I cannot do.") or "20 bots" in isdone:
        ...
        sys.exit(1)
    await mmo_bot.send_message(bf, name)
    await asyncio.sleep(1)
    isdone = (await mmo_bot.get_messages(bf, limit=1))[0].text
    if not isdone.startswith("Good."):
        await mmo_bot.send_message(bf, "My Assistant Bot")
        await asyncio.sleep(1)
        isdone = (await mmo_bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.critical(
            ...
            sys.exit(1)
    await mmo_bot.send_message(bf, username)
    await asyncio.sleep(1)
    isdone = (await mmo_bot.get_messages(bf, limit=1))[0].text
    await mmo_bot.send_read_acknowledge("botfather")
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "mmo_" + (str(who.id))[6:] + str(ran) + "_bot"
        await mmo_bot.send_message(bf, username)
        await asyncio.sleep(1)
        nowdone = (await mmo_bot.get_messages(bf, limit=1))[0].text
        if nowdone.startswith("Done!"):
            token = nowdone.split("`")[1]
            udB.set_key("BOT_TOKEN", token)
            await enable_inline(mmo_bot, username)
            LOGS.info(
...
async def autopilot():
    from .. import asst, udB, mmo_bot

    channel = udB.get_key("LOG_CHANNEL")
    new_channel = None
    if channel:
        try:
            chat = await mmo_bot.get_entity(channel)
        except BaseException:
    ...
    assistant = True
    try:
        await mmo_bot.get_permissions(int(channel), asst.me.username)
    except UserNotParticipantError:
        try:
            await mmo_bot(InviteToChannelRequest(int(channel), [asst.me.username]))
        except BaseException as er:
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        ...
        ll = await mmo_bot.upload_file(photo)
        try:
            await mmo_bot(
...
async def customize():
    ...
    try:
        ...
        if not mmo_bot.me.username:
            sir = mmo_bot.me.first_name
        else:
            sir = f"@{mmo_bot.me.username}"
        file = random.choice(
            [
                "https://telegra.ph/file/92cd6dbd34b0d1d73a0da.jpg",
                "https://telegra.ph/file/a97973ee0425b523cdc28.jpg",
                "resources/extras/mmo_assistant.jpg",
            ]
        )
        ...
        await asyncio.sleep(1)
        await mmo_bot.send_message("botfather", "/cancel")
        await asyncio.sleep(1)
        await mmo_bot.send_message("botfather", "/setuserpic")
        await asyncio.sleep(1)
        isdone = (await mmo_bot.get_messages("botfather", limit=1))[0].text
        if isdone.startswith("Invalid bot"):
            LOGS.info("Error while trying to customise assistant, skipping...")
            return
        await mmo_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await mmo_bot.send_file("botfather", file)
        await asyncio.sleep(2)
        await mmo_bot.send_message("botfather", "/setabouttext")
        await asyncio.sleep(1)
        await mmo_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await mmo_bot.send_message(
            "botfather", f"✨ Hello ✨!! I'm Assistant Bot of {sir}"
        )
        await asyncio.sleep(2)
        await mmo_bot.send_message("botfather", "/setdescription")
        await asyncio.sleep(1)
        await mmo_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await mmo_bot.send_message(
...
async def plug(plugin_channels):
    ...
    for chat in plugin_channels:
        LOGS.info(f"{'•'*4} {chat}")
        try:
            async for x in mmo_bot.iter_messages(
...
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
    ...
    try:
        ...
        try:
            await (await mmo_bot.send_message(chat_id, str(e))).delete()
            spam_sent = await asst.send_message(chat_id, MSG, file=PHOTO, buttons=BTTS)
        ...
        LOGS.info(el)
        try:
            spam_sent = await mmo_bot.send_message(chat_id, MSG)
    ...
    try:
        ...
        await mmo_bot(JoinChannelRequest("MHMUser"))
...
async def WasItRestart(udb):
    ...
    if not key:
        return
    from .. import asst, mmo_bot

    try:
        data = key.split("_")
        who = asst if data[0] == "bot" else mmo_bot
...
async def enable_inline(mmo_bot, username):
    bf = "BotFather"
    await mmo_bot.send_message(bf, "/setinline")
    await asyncio.sleep(1)
    await mmo_bot.send_message(bf, f"@{username}")
    await asyncio.sleep(1)
    await mmo_bot.send_message(bf, "Search")
    await mmo_bot.send_read_acknowledge(bf)
